...
void doit(int clientfd)
{
  ...
  // sprintf(response_buf, "%sServer: Tiny Web Server\r\n", response_buf);                            // 서버 이름
  // sprintf(response_buf, "%sConnection: close\r\n", response_buf);                                  // 연결 방식
  // sprintf(response_buf, "%sContent-length: %d\r\n\r\n", response_buf, content_length); // 컨텐츠 길이
  // Rio_writen(clientfd, response_buf, strlen(response_buf));
